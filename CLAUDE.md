...
| A community admin edits their community, not the map | `geo_entities_update_admin` admits `administers_geo(id)`, which grants the whole ROW -- so a community_admin could reparent their village anywhere, or set `kind='lga'` with `parent_id=null` and promote it to a second Local Government Area at the root, since `administers_geo(id)` is evaluated on the row's own id and the id never changes. Migration 037's guard restores `parent_id`, `kind`, `slug`, `merged_into_id`, `status` and `deleted_at` for anyone who is not a platform admin. They keep name, aliases, description, coordinates and sort order -- which is the point of the role |
| The directory holds exactly ONE Local Government Area | `geo_entities_root_only_lga` enforces `kind='lga' <-> parent_id is null` and therefore permits a SECOND root. `/communities` renders every root as a top-level branch, so the first symptom would be the whole directory appearing twice. A trigger, not a CHECK, because a CHECK cannot see other rows |
| A community admin's editing surface is the community's own page | `is_staff()` is super_admin/admin/moderator, so a community_admin cannot reach `/admin` at all. Putting the control on `/communities/[slug]` also puts it where they are standing when they notice the misspelling |
| `updateCommunityAction` is NOT gated on `requireAdmin()` | the whole point of community_admin is that the person in Umuida knows how Umuida is spelled. The server checks only that somebody is signed in and lets the DATABASE decide; the row count afterwards is what turns a filtered refusal into an error message rather than a false success |
