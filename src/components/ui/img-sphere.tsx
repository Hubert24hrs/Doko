 ...
 *
 * Displays member images arranged in a 3D sphere layout using Fibonacci sphere distribution.
 * Supports drag-to-rotate, momentum physics, auto-rotation, verification badge overlays,
 * and custom click callbacks.
 */
...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const renderImageNode = useCallback((image: ImageData, index: number) => {
    ...
    return (
      ...
      >
        <div className="relative w-full h-full rounded-full overflow-hidden shadow-lg border-2 border-white/30 dark:border-border">
          <img
            src={image.src}
            alt={image.alt}
        ...
        {image.verificationType ? (
          <div className="absolute -bottom-1 -right-1 z-20 rounded-full bg-surface shadow-xs p-0.5 pointer-events-none">
            <VerifiedBadge type={image.verificationType} />
          </div>
        ) : null}
      </div>
    );
  }, [worldPositions, baseImageSize, containerSize, hoveredIndex, onImageClick]);

  const renderSpotlightModal = () => {
    if (!selectedImage || onImageClick) return null;
    ...
    return (
      ...
      >
        <div
          className="bg-surface rounded-xl max-w-md w-full overflow-hidden shadow-2xl border border-border"
        ...
        >
          <div className="relative aspect-square">
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-2 right-2 size-8 bg-black/60 rounded-full text-white flex items-center justify-center hover:bg-black/80 transition-all cursor-pointer"
            >
              <X size={16} />
            </button>
          </div>
            ...
            <div className="p-5">
              {selectedImage.title && (
                <div className="flex items-center justify-between mb-2">
                  ...
                  {selectedImage.verificationType ? (
                    <VerifiedBadge type={selectedImage.verificationType} ticker />
                  ) : null}
                </div>
              )}
  ...
  if (!isMounted) {
    return (
      <div
        className="bg-surface-sunken rounded-xl animate-pulse flex items-center justify-center border border-border"
        style={{ width: containerSize, height: containerSize }}
      >
        <div className="text-muted-foreground text-sm font-medium">Loading Community Sphere...</div>
      </div>
    );
  }
  ...
  if (!images.length) {
    return (
      <div
        className="bg-surface-sunken/50 rounded-xl border-2 border-dashed border-border flex items-center justify-center"
        style={{ width: containerSize, height: containerSize }}
      >
        <div className="text-muted-foreground text-center p-6">
          <p className="font-semibold text-foreground text-sm">No active members in this window</p>
          <p className="text-xs mt-1">Verified members active within 24h appear here.</p>
        </div>
      </div>
    );
  }
