...
/**
 * Picks photos and holds them until the issue exists.
 *
 * The same two-step shape as post images, and for the same reason: the
 * storage policy authorises against the issue's reporter, so there is nothing
 * to authorise against until an issue id exists. See
...
export function ImageUploader({
  ...
  async function handleFiles(fileList: FileList | null) {
    ...
    for (const file of incoming) {
      if (images.length + accepted.length >= MAX_IMAGES_PER_ISSUE) {
        rejected.push(
          `An issue can carry at most ${MAX_IMAGES_PER_ISSUE} photos — ${file.name} was not added.`,
        );
        continue;
      }
  ...
  const atLimit = images.length >= MAX_IMAGES_PER_ISSUE;
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          ...
          {atLimit ? `Limit of ${MAX_IMAGES_PER_ISSUE} reached` : "Add photos"}
        </label>

        <span className="text-xs text-muted-foreground">
          Up to {MAX_IMAGES_PER_ISSUE}, {formatBytes(MAX_IMAGE_BYTES)} each. A
          clear photograph usually settles what a description cannot.
        </span>
      </div>
    </div>
  );
...
/**
 * Uploads staged photos to storage and records them against the issue.
 *
 * Runs after the issue is created, because the storage policy authorises
 * against the issue's reporter. Each file is attempted independently: one
 * failure loses one photo, not the report and not the rest.
 */
export async function uploadPending(
  ...
  for (const image of images) {
    try {
      ...
      const path = buildStoragePath(issueId, image.file.type);
      ...
      console.error("[issues.media.upload] unexpected", image.file.name, cause);
      failed.push(image.file.name);
    } finally {
