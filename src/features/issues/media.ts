...
/**
 * Image constraints for issue photos. Mirrors features/posts/media.ts --
 * see that file for why these are duplicated across three layers rather than
 * trusted to one.
 */

export const MAX_IMAGE_BYTES = 8 * 1024 * 1024; // 8 MB
export const MAX_IMAGES_PER_ISSUE = 4;
...
/**
 * '<issue_id>/<uuid>.<ext>'. The leading segment is what
 * storage_path_issue_id() reads to decide who may fetch the bytes.
 */
export function buildStoragePath(
  ...
  return `${issueId}/${crypto.randomUUID()}.${extension}`;
}
